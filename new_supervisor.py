...
def new_supervisor():
    ...
    reg_no_label = Label(window, text="UID:")
...
def register(data):
    try:
        ...
        cur.execute("CREATE TABLE IF NOT EXISTS Supervisor (name TEXT, uid TEXT, specialization TEXT, phone TEXT, email TEXT, password TEXt)")
        # Insert the data into the table
        cur.execute(f"INSERT INTO Supervisor (name, uid, specialization, phone, email, password) VALUES('{data[0].get()}', '{data[1].get()}', '{data[2].get()}', '{data[3].get()}', '{data[4].get()}', '{data[5].get()}')")
        # Commit the changes
        db.commit()
